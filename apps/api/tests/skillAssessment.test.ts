...
describe('skill assessment API', () => {
  ...
  it('starts a diagnostic assessment with 12 ready MCQs', async () => {
    const guestSessionId = crypto.randomUUID();
    const res = await request(app)
      .post('/skill-assessments/diagnostic')
      .send({ topic: 'Artificial Intelligence', guestSessionId });

    expect(res.status).toBe(201);
    expect(res.body.assessment.status).toBe('ready');
    expect(res.body.assessment.questions).toHaveLength(12);
  ...
  it('allows guest submit and returns level + score without login', async () => {
    const guestSessionId = crypto.randomUUID();
    const assessment = await generateSkillAssessment(
      { topic: 'Natural Language Processing', guestSessionId },
      undefined,
      'free',
      fakeGen,
    );
    ...
    const submission = await submitSkillAssessment(assessment!.id, answers, { guestSessionId });
  ...
  it('requires auth or guest session to submit and returns level + score', async () => {
    ...
    const submission = await submitSkillAssessment(assessment!.id, answers, { userId });
    expect(submission.score).toBe(100);
    expect(submission.level).toBe('Expert');
  ...
  it('rejects duplicate submission', async () => {
    ...
    await submitSkillAssessment(assessment!.id, answers, { userId });
    await expect(submitSkillAssessment(assessment!.id, answers, { userId })).rejects.toMatchObject({
      statusCode: 409,
    });
  });
  ...
  it('marks guest assessment completed after authenticated submit', async () => {
    ...
    await submitSkillAssessment(assessment!.id, answers, { userId, guestSessionId });
