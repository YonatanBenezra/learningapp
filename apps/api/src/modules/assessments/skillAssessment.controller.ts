...
export const startDiagnostic = asyncHandler(async (req, res) => {
  const tier = await resolveRequestTier(req.user?.id, req.user?.tier);
  const assessment = await service.startDiagnosticAssessment(req.body, req.user?.id, tier);
...
export const submit = asyncHandler(async (req, res) => {
  const submission = await service.submitSkillAssessment(req.params.id, req.body.answers, {
    userId: req.user?.id,
...
export const getResult = asyncHandler(async (req, res) => {
  const guestSessionId = req.query.guestSessionId as string | undefined;
  const submission = await service.getSkillAssessmentResult(req.params.id, {
    userId: req.user?.id,
    guestSessionId,
  });
  res.json({ submission });
