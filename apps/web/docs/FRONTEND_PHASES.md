...
Companion to `apps/api/docs/BACKEND_PHASES.md`.
...
- ✅ **FP5 — Courses & Structure** (done, **learner-facing against the real backend**, styled with the new indigo tokens + sidebar shell): `coursesApi.getStructure` + `CourseStructure` types + `useCourseStructure`; shared `CourseCard` (status-colored, progress); **`/courses`** list (grid + skeletons + error-retry + empty state → create-first-course); **`/courses/[courseId]`** overview via `CourseOverview` — poll-while-generating, failed→reason+create-new, not-found, and ready→header (progress, level, category) + **modules → lessons list** (each lesson links to `/lesson/[id]`) + "View as diagram"; **`/courses/[courseId]/structure`** → **`CourseFlowGraph`** (@xyflow/react) rendering the Course→Module→Lesson tree with manual layout, Background/Controls/MiniMap, **click a lesson node → `/lesson/[id]`**, and a not-ready guard. Next 16 async `params` handled via server page → client component. Verified **typecheck 0 · lint 0 errors · `next build` green** (13 routes; `/courses/[courseId]` + `/structure` dynamic). Direction confirmed with user: **learner course pages, real backend** (app was concurrently reworked toward an LMS-admin look — landing stays teal, app tokens now indigo; course pages use tokens so they match whatever theme is active). Note: not yet browser-driven against a live backend.
- ✅ **FP6 — Lessons & Progress** (done, real backend): `src/domain/progress.ts` (`LessonProgress`/`LessonProgressStatus`); `lessonsApi` (`getLesson`, `startLesson`, `completeLesson` + `LessonContent`/`CompleteLessonResult` types); `useLesson` / `useStartLesson` (cache-patch, no refetch) / `useCompleteLesson` (invalidates `lesson`/`courses`/`course:id`(+structure via prefix)/`progress`/`gamification`, and **syncs the streak into the auth store** so the topbar updates). **`/lesson/[lessonId]`** → `LessonView`: **auto-marks in-progress on open** (once per lesson id, ref-guarded effect), renders the AI `content.summary` as prose (empty-state fallback), **prev/next lesson navigation** computed from the flattened course structure (+ "Lesson n of N" + module breadcrumb), **Mark-as-complete** with pending/error states, and a **completion feedback banner** (🔥 streak, course-progress %, unlocked-achievement badges) shown via `mutation.variables === lessonId` so it self-clears on navigation. Verified **typecheck 0 · lint 0 errors · build green** (`/lesson/[lessonId]` dynamic). Caveat: contract-verified, not yet browser-driven against a live backend.
- ✅ **FP7 — Assessments (quizzes + exams)** (done, real backend): `domain/assessment.ts` (`AssessmentQuestion` — the answer-stripped taking shape — `Quiz`, `Exam`, `GradedResult`, `AssessmentSubmission`); `assessmentsApi` (quiz: generate `POST /lessons/:id/quizzes`, `GET /quizzes/:id`, submit; exam: generate `POST /modules/:id/exam` + `POST /courses/:id/exam`, `GET /exams/:id`, submit) unwrapping the `{quiz|exam|submission}` envelopes; hooks `useGenerateQuiz`/`useQuiz`/`useSubmitQuiz` + scope-aware `useGenerateExam`/`useExam`/`useSubmitExam`. **Shared runner UI** (quizzes and exams share one schema): `AssessmentView` (holds answer state, renders MCQ radios + short-answer textareas, "n of m answered", submit) → `ResultSummary` (score with pass/almost/practice tone, per-question ✓/✗ with **correct answer revealed + feedback**, "Try another" regenerate); thin `QuizRunner`/`ExamRunner` wrappers (loading/not-found via shared `shell`). Pages wired: **`/lesson/[lessonId]/quiz/[quizId]`** and **`/exam/[examId]`**. Entry points added: **"Take a quiz"** on `LessonView` (generate → navigate, with quota-limit error copy), **"Course exam"** + per-module **"Exam"** on `CourseOverview` (scope-aware generate → navigate, per-button pending via `mutation.variables.scopeId`). Verified **typecheck 0 · lint 0 errors · build green** (`/exam/[examId]`, `/lesson/[lessonId]/quiz/[quizId]` dynamic; 12 pre-existing warnings all in teammate files). Caveat: contract-verified, not yet browser-driven against a live backend.
- ⬜ FP8–FP13 — pending.
