...
import { assessmentStartRedirect } from '@/src/features/skill-assessment/assessmentGate';
...
export function RequireAssessmentComplete({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    router.replace(assessmentStartRedirect(pathname));
  }, [loading, completed, router, pathname]);

  if (loading) return <AuthWorkspaceLoader />;
  if (!completed) return <AuthWorkspaceLoader redirecting />;
  return <>{children}</>;
}
