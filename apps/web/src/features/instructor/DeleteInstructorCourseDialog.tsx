      ...
      >
            ...
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-bad">
                  {t('instructor.deleteCourseTitle')}
                </p>
        ...
        <div className="px-6 py-5">
          <p className="text-sm leading-6 text-ink-2">
            {t('instructor.deleteCourseWarning')}{' '}
            <strong className="font-semibold text-ink">{course.title}</strong>
          </p>

          {course.enrollmentCount > 0 ? (
            <p className="mt-4 rounded-lg border border-warn/20 bg-warn-soft px-4 py-3 text-sm text-warn">
              {t('instructor.deleteCourseSalesWarning', {
                count: String(course.enrollmentCount),
              })}
            </p>
          ) : null}
