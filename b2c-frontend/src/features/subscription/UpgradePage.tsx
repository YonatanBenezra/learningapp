  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-line bg-bg-elev shadow-card">
      <div className="border-b border-line px-5 py-4 sm:px-6">
        <h2 className="text-lg font-semibold text-ink">{t('marketing.comparePlans')}</h2>
        <p className="mt-1 text-sm text-ink-2">{t('marketing.compareIntro')}</p>
      </div>
          ...
          {rows.map((row, index) => (
            ...
            >
              ...
              <span className="text-center text-ink-2">
                <ComparisonCell value={row.free} yesLabel={yesLabel} dashLabel={dashLabel} />
              </span>
              <span className="text-center text-ink-2">
                <ComparisonCell value={row.standard} yesLabel={yesLabel} dashLabel={dashLabel} />
              </span>
              <span className="text-center font-medium text-primary">
                <ComparisonCell value={row.premium} yesLabel={yesLabel} dashLabel={dashLabel} />
              </span>
            </div>
          ))}
        </div>
...
function UpgradeFaq() {
  ...
  return (
    <section className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-ink">{t('marketing.pricingFaqTitle')}</h2>
        <p className="mt-1 text-sm text-ink-2">{t('marketing.pricingFaqIntro')}</p>
      </div>
...
export function UpgradePage() {
  ...
  const standardPlan = pricingPlans.find((p) => p.id === 'standard')!;
    ...
    return (
      <div className={shellClass}>
        <div className="rounded-2xl border border-line bg-bg-elev px-6 py-14 text-center shadow-card">
          <p className="text-sm text-ink-2">{t('subscription.loadError')}</p>
  ...
  const trialEnds = subscription.trialEndsAt ? formatDate(subscription.trialEndsAt) : null;
    ...
    <div className={shellClass}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-2xl">
          <h1 className="text-2xl font-bold tracking-tight text-ink sm:text-3xl">
            {isPaid
              ? isPremium
                ? t('subscription.yourPremiumPlan')
                : t('subscription.yourStandardPlan')
              : t('subscription.upgradeTitle')}
          </h1>
          <p className="mt-2 text-sm leading-7 text-ink-2 sm:text-base">
            {isPaid
              ? t('subscription.manageBillingDesc')
              : subscription.requiresPayment
                ? t('subscription.trialEndedDesc')
                : t('subscription.trialIntro', { months: String(TRIAL_PERIOD_MONTHS) })}
          </p>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary-soft/40 px-5 py-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-ink-2">
              {trialEnds ? t('subscription.trialEndsOn', { date: trialEnds }) : null}{' '}
              {t('dashboard.trialPremiumNote')}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {!isPaid && subscription.requiresPayment ? (
        <div className="rounded-xl border border-bad/20 bg-bad-soft px-5 py-4 text-sm leading-6 text-ink">
          {t('subscription.trialEndedBanner', { months: String(TRIAL_PERIOD_MONTHS) })}
        </div>
      ) : null}
      ...
      {isPaid ? (
        <section className="rounded-2xl border border-line bg-bg-elev p-6 shadow-card sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm text-ink-2">{t('subscription.currentPlan')}</p>
              <p className="mt-2 text-2xl font-bold text-ink">
                {isPremium ? premiumPlan.name : standardPlan.name}
              </p>
              {periodEnd ? (
                <p className="mt-2 text-sm text-ink-2">
                  {subscription.cancelAtPeriodEnd
                    ? t('subscription.accessUntil', { date: periodEnd })
          ...
          <PlanCard
            ...
            badge={
              subscription.trialActive
                ? t('subscription.activeTrial')
                : subscription.requiresPayment
                  ? t('subscription.expired')
                  : t('subscription.trialBadge')
            }
            current
            cta={t('subscription.currentPlanBtn')}
            recommendedLabel={t('marketing.recommended')}
            currentPlanLabel={t('subscription.currentPlanBtn')}
          />
          <PlanCard
            name={standardPlan.name}
            subtitle={standardPlan.subtitle}
            price={STANDARD_PRICE_USD}
            period={standardPlan.period}
            features={standardPlan.features.slice(1)}
            cta={
              subscription.requiresPayment
                ? t('subscription.subscribeStandard')
                : t('subscription.chooseStandard')
            }
      ...
      {isStandard && !isPremium ? (
        <section className="rounded-2xl border border-primary/30 bg-bg-elev p-6 shadow-card ring-1 ring-primary/10 sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-sm font-medium text-primary">{t('subscription.upgradePremium')}</p>
              <h2 className="mt-1 text-xl font-bold text-ink">{t('subscription.unlockUnlimited')}</h2>
              <p className="mt-2 text-sm leading-6 text-ink-2">{t('subscription.unlockUnlimitedDesc')}</p>
            ...
            <div className="shrink-0 text-center lg:text-right">
              <p className="text-3xl font-bold text-ink">
                ${PREMIUM_PRICE_USD}
                <span className="text-sm font-medium text-ink-3"> / {premiumPlan.period}</span>
              </p>
              <Button
      ...
      {actionError ? (
        <p className="rounded-xl border border-bad/30 bg-bad-soft px-4 py-3 text-sm text-bad">
          {mutationMessage(actionError, t('subscription.mutationError'))}
        </p>
      ) : null}
      ...
      {!isPaid || isStandard ? (
        <p className="text-center text-sm text-ink-3">{t('subscription.stripeNote')}</p>
      ) : null}
