  ...
  if (graded) {
    return (
      <Badge variant="good" className="gap-1.5">
        <CheckCircle2 className="size-3.5" /> Graded
      </Badge>
    );
  }
  if (grading) {
    return (
      <Badge variant="warn" className="gap-1.5">
        <Loader2 className="size-3.5 animate-spin" /> AI grading
      </Badge>
    );
  }
  return (
    ...
    <section className="overflow-hidden rounded-xl border border-line bg-bg-elev shadow-soft">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-bold text-ink">Submission graded</h2>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs font-semibold uppercase tracking-[0.1em] text-ink-3">Score</p>
      ...
      {submission.feedback ? (
        <div className="px-5 py-5 sm:px-6">
          <p className="text-xs font-semibold uppercase tracking-[0.1em] text-ink-3">Feedback</p>
...
export function ExerciseView({
  ...
  const lessonTitle = lessonQ.data?.lesson.title ?? 'Lesson';
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
        <div className="border-b border-line pb-5">
          ...
          <nav className="mt-3 flex flex-wrap items-center gap-1.5 text-sm text-ink-3">
            ...
            <ChevronRight className="size-4" />
            <span className="font-medium text-ink">Hands-on exercise</span>
          </nav>
        </div>
        ...
        <section className="overflow-hidden rounded-xl border border-line bg-bg-elev shadow-soft">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-bg-soft/60 px-4 py-3.5 sm:px-5">
            ...
            {graded ? (
              <span className="text-xs font-medium text-ink-3">Read-only after submission</span>
            ) : (
              <span className="text-xs font-medium text-ink-3">Complete the task below, then submit</span>
            )}
          </div>
        ...
        {submissionQ.isLoading && submissionId ? (
          <section className="rounded-xl border border-line bg-bg-elev p-5 shadow-soft sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-ink">Evaluating your submission</p>
                <p className="text-sm text-ink-2">AI grading usually completes within a few seconds.</p>
              </div>
            </div>
        ...
        {!graded ? (
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-ink-2">
                  Your work will be reviewed by AI and scored against the exercise rubric.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                {submit.isError ? (
                  <p className="text-sm text-bad">Could not submit. Please try again.</p>
                ) : null}
                <Button
                  onClick={onSubmit}
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={`/lesson/${lessonId}`}>
              <Button variant="soft">
                <ArrowLeft className="size-4" /> Return to lesson
              </Button>
            </Link>
          </div>
        )}
