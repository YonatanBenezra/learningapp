...
function LabPathLogo({ tour = false }: { tour?: boolean }) {
  return (
    <Link
      href="/"
      data-tour={tour ? 'tour-nav-home' : undefined}
...
function NavLink({
  ...
  return (
    <Link href={link.href} data-tour={navTourId(link.href)} onClick={handleClick} className={classes}>
      {link.label}
    </Link>
  );
}
